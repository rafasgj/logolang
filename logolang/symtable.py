...
def current_scope():
    return ".".join("@%s" % scope["name"] for scope in __symtable["scopes"] if scope["name"])
...
def add_symbol(symbol, symtype, lineno=-1, **kwargs):
    ...
    if sym:
        ...
        sym = {"name": symbol, "type": symtype, "lineno": lineno, "usage": 0, "scope": sym_scope, "fqsn": fqsn, **kwargs}
        logging.log(5, "Adding symbol:%s:%s:%d:%s", symbol, symtype, lineno, repr(kwargs))
        __symtable["scopes"][-1]["symbols"][symbol.upper()] = sym
    
    arg_scope = f"@{sym['name']}"
    if sym["scope"]:
        arg_scope = sym['scope'] + arg_scope
    args = [arg_scope + f".{arg}" if not arg.startswith("@") else arg for arg in sym.get("argv", [])]
    sym["argv"] = args
    return sym
...
def increase_symbol_usage(symbol, symtype=None):
    ...
    for arg in sym.get("argv",[]) + sym.get("uses", []):
        if sym['scope'] and not arg.startswith("@"):
            fqsn = f"@{'.@'.join(lvl for lvl in [sym['scope'], sym['name']] if lvl)}.{arg}"
